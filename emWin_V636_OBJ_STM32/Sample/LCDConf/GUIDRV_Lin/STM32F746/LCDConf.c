...
*/
  ...
  #define LCD_FRAME_BUFFER0 ((U32)_aVRAM0)
  #if GUI_NUM_LAYERS > 1
  #define LCD_FRAME_BUFFER1 ((U32)_aVRAM1)
  #endif
#else
  #define LCD_FRAME_BUFFER0 ((U32)0xC0200000)
  #if GUI_NUM_LAYERS > 1
  #define LCD_FRAME_BUFFER1 ((U32)LCD_FRAME_BUFFER0 + XSIZE_PHYS * YSIZE_PHYS * sizeof(U32) * NUM_VSCREENS * NUM_BUFFERS)
  #endif
#endif
...
#define GPIO_AF9_LTDC                ((U8)0x09)                   // LCD-TFT Alternate Function 9  mapping
#define GPIO_AF14_LTDC               ((U8)0x0E)                   // LCD-TFT Alternate Function 14 mapping
...
*/
typedef struct {
  volatile U32 HSPolarity;             // Horizontal synchronization polarity.        Can be one value of @ref LTDC_HSPolarity
  volatile U32 VSPolarity;             // Vertical synchronization polarity.          Can be one value of @ref LTDC_VSPolarity
  volatile U32 DEPolarity;             // Data enable polarity.                       Can be one of value of @ref LTDC_DEPolarity
  volatile U32 PCPolarity;             // Pixel clock polarity.                       Can be one of value of @ref LTDC_PCPolarity
  volatile U32 HorizontalSync;         // Number of Horizontal synchronization width. Must range from 0x000 to 0xFFF.
  volatile U32 VerticalSync;           // Number of Vertical synchronization heigh.   Must range from 0x000 to 0x7FF.
  volatile U32 AccumulatedHBP;         // Accumulated horizontal back porch width.    Must range from LTDC_HorizontalSync to 0xFFF.
  volatile U32 AccumulatedVBP;         // Accumulated vertical back porch heigh.      Must range from LTDC_VerticalSync to 0x7FF.
  volatile U32 AccumulatedActiveW;     // Accumulated active width.                   Must range from LTDC_AccumulatedHBP to 0xFFF.
  volatile U32 AccumulatedActiveH;     // Accumulated active heigh.                   Must range from LTDC_AccumulatedVBP to 0x7FF.
  volatile U32 TotalWidth;             // Total width.                                Must range from LTDC_AccumulatedActiveW to 0xFFF.
  volatile U32 TotalHeigh;             // Total heigh.                                Must range from LTDC_AccumulatedActiveH to 0x7FF.
  volatile U32 BackgroundRedValue;     // Background red value.                       Must range from 0x00 to 0xFF.
  volatile U32 BackgroundGreenValue;   // Background green value.                     Must range from 0x00 to 0xFF.
  volatile U32 BackgroundBlueValue;    // Background blue value.                      Must range from 0x00 to 0xFF.
} LTDC_INIT_T;

typedef struct {
  volatile U32 HorizontalStart;        // Window Horizontal Start Position.           Must range from 0x000 to 0xFFF.
  volatile U32 HorizontalStop;         // Window Horizontal Stop Position.            Must range from 0x0000 to 0xFFFF.
  volatile U32 VerticalStart;          // Window vertical Start Position.             Must range from 0x000 to 0xFFF.
  volatile U32 VerticalStop;           // Window vaertical Stop Position.             Must range from 0x0000 to 0xFFFF.
  volatile U32 PixelFormat;            // Pixel format.                               Can be one of value of @ref LTDC_Pixelformat
  volatile U32 ConstantAlpha;          // Constant alpha used for blending.           Must range from 0x00 to 0xFF.
  volatile U32 DefaultColorBlue;       // Default blue value.                         Must range from 0x00 to 0xFF.
  volatile U32 DefaultColorGreen;      // Default green value.                        Must range from 0x00 to 0xFF.
  volatile U32 DefaultColorRed;        // Default red value.                          Must range from 0x00 to 0xFF.
  volatile U32 DefaultColorAlpha;      // Default alpha value.                        Must range from 0x00 to 0xFF.
  volatile U32 BlendingFactor_1;       // Blending factor 1.                          Can be one of value of @ref LTDC_BlendingFactor1
  volatile U32 BlendingFactor_2;       // Blending factor 2.                          Can be one of value of @ref LTDC_BlendingFactor2
  volatile U32 CFBStartAdress;         // Color frame buffer address
  volatile U32 CFBLineLength;          // Color frame buffer line length.             Must range from 0x0000 to 0x1FFF.
  volatile U32 CFBPitch;               // Color frame buffer pitch in bytes.          Must range from 0x0000 to 0x1FFF.
  volatile U32 CFBLineNumber;          // Number of line in frame buffer.             Must range from 0x000 to 0x7FF.
} LTDC_INIT_LAYER_T;
...
#if (defined __SES_ARM)   // SES
  static U8 _aVRAM0[XSIZE_PHYS * YSIZE_PHYS * NUM_BUFFERS * NUM_VSCREENS * BYTE_PER_PIXEL_0] __attribute__ ((section (".GUI_RAM")));
  #if GUI_NUM_LAYERS > 1
    static U8 _aVRAM1[XSIZE_1 * YSIZE_1 * NUM_BUFFERS * NUM_VSCREENS * BYTE_PER_PIXEL_1] __attribute__ ((section (".GUI_RAM")));
  #endif
#else
#endif

static LTDC_LAYER_T * _apLayer[] = {
  LTDC_LAYER1,
#if GUI_NUM_LAYERS > 1
  LTDC_LAYER2
#endif
...
static void _LCD_AF_GPIOConfig(void) {
  ...
  //
  _RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOG | RCC_AHB1Periph_GPIOE | \
                         RCC_AHB1Periph_GPIOI | RCC_AHB1Periph_GPIOJ | \
                         RCC_AHB1Periph_GPIOK,  ENABLE);
  //
  // GPIOE configuration
  //
  _GPIO_PinAFConfig(GPIOE, GPIO_PinSource4, GPIO_AF14_LTDC); // LCD_B0
  GPIO_InitStruct.GPIO_Pin   = GPIO_Pin_4;
  GPIO_InitStruct.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStruct.GPIO_Mode  = GPIO_Mode_AF;
  GPIO_InitStruct.GPIO_OType = GPIO_OType_PP;
  GPIO_InitStruct.GPIO_PuPd  = GPIO_PuPd_NOPULL;
  _GPIO_Init(GPIOE, &GPIO_InitStruct);
  //
  // GPIOG configuration
  //
  _GPIO_PinAFConfig(GPIOG, GPIO_PinSource12, GPIO_AF9_LTDC); // LCD_B4
  GPIO_InitStruct.GPIO_Pin   = GPIO_Pin_12;
  GPIO_InitStruct.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStruct.GPIO_Mode  = GPIO_Mode_AF;
  GPIO_InitStruct.GPIO_OType = GPIO_OType_PP;
  GPIO_InitStruct.GPIO_PuPd  = GPIO_PuPd_NOPULL;
  _GPIO_Init(GPIOG, &GPIO_InitStruct);
  //
  // GPIOI configuration
  //
  _GPIO_PinAFConfig(GPIOI, GPIO_PinSource9,  GPIO_AF14_LTDC);
  _GPIO_PinAFConfig(GPIOI, GPIO_PinSource10, GPIO_AF14_LTDC);
  _GPIO_PinAFConfig(GPIOI, GPIO_PinSource13, GPIO_AF14_LTDC);
  _GPIO_PinAFConfig(GPIOI, GPIO_PinSource14, GPIO_AF14_LTDC);
  _GPIO_PinAFConfig(GPIOI, GPIO_PinSource15, GPIO_AF14_LTDC); // LCD_R0
  GPIO_InitStruct.GPIO_Pin   = GPIO_Pin_9 | GPIO_Pin_10 | GPIO_Pin_13 | GPIO_Pin_14 | GPIO_Pin_15;
  GPIO_InitStruct.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStruct.GPIO_Mode  = GPIO_Mode_AF;
  GPIO_InitStruct.GPIO_OType = GPIO_OType_PP;
  GPIO_InitStruct.GPIO_PuPd  = GPIO_PuPd_NOPULL;
  ...
  // GPIOJ configuration
  //
  _GPIO_PinAFConfig(GPIOJ, GPIO_PinSource0, GPIO_AF14_LTDC);  // LCD_R1
  _GPIO_PinAFConfig(GPIOJ, GPIO_PinSource1, GPIO_AF14_LTDC);  // LCD_R2
  _GPIO_PinAFConfig(GPIOJ, GPIO_PinSource2, GPIO_AF14_LTDC);  // LCD_R3
  _GPIO_PinAFConfig(GPIOJ, GPIO_PinSource3, GPIO_AF14_LTDC);  // LCD_R4
  _GPIO_PinAFConfig(GPIOJ, GPIO_PinSource4, GPIO_AF14_LTDC);  // LCD_R5
  _GPIO_PinAFConfig(GPIOJ, GPIO_PinSource5, GPIO_AF14_LTDC);  // LCD_R6
  _GPIO_PinAFConfig(GPIOJ, GPIO_PinSource6, GPIO_AF14_LTDC);  // LCD_R7
  _GPIO_PinAFConfig(GPIOJ, GPIO_PinSource7, GPIO_AF14_LTDC);  // LCD_G0
  _GPIO_PinAFConfig(GPIOJ, GPIO_PinSource8, GPIO_AF14_LTDC);  // LCD_G1
  _GPIO_PinAFConfig(GPIOJ, GPIO_PinSource9, GPIO_AF14_LTDC);  // LCD_G2
  _GPIO_PinAFConfig(GPIOJ, GPIO_PinSource10, GPIO_AF14_LTDC); // LCD_G3
  _GPIO_PinAFConfig(GPIOJ, GPIO_PinSource11, GPIO_AF14_LTDC); // LCD_G4
  _GPIO_PinAFConfig(GPIOJ, GPIO_PinSource13, GPIO_AF14_LTDC); // LCD_B1
  _GPIO_PinAFConfig(GPIOJ, GPIO_PinSource14, GPIO_AF14_LTDC); // LCD_B2
  _GPIO_PinAFConfig(GPIOJ, GPIO_PinSource15, GPIO_AF14_LTDC); // LCD_B3
  ...
  // GPIOK configuration
  //
  _GPIO_PinAFConfig(GPIOK, GPIO_PinSource0, GPIO_AF14_LTDC); // LCD_G5
  _GPIO_PinAFConfig(GPIOK, GPIO_PinSource1, GPIO_AF14_LTDC); // LCD_G6
  _GPIO_PinAFConfig(GPIOK, GPIO_PinSource2, GPIO_AF14_LTDC); // LCD_G7
  _GPIO_PinAFConfig(GPIOK, GPIO_PinSource4, GPIO_AF14_LTDC); // LCD_B5
  _GPIO_PinAFConfig(GPIOK, GPIO_PinSource5, GPIO_AF14_LTDC); // LCD_B6
  _GPIO_PinAFConfig(GPIOK, GPIO_PinSource6, GPIO_AF14_LTDC); // LCD_B7
  _GPIO_PinAFConfig(GPIOK, GPIO_PinSource7, GPIO_AF14_LTDC);
...
static void _DMA_AlphaBlendingBulk(LCD_COLOR * pColorFG, LCD_COLOR * pColorBG, LCD_COLOR * pColorDst, U32 NumItems) {
  ...
  #pragma GCC push_options     // Something is going wrong in this function when build with optimizations.
  #pragma GCC optimize ("O0")  // So we set them to 0 for this function.
#endif
...
static void _LCD_DisplayOn(void) {
  ...
  //
  _GPIO_SetBits(GPIOI, GPIO_Pin_12);
  _GPIO_SetBits(GPIOK, GPIO_Pin_3);
  //
  // Display On
  //
...
static void _LCD_DisplayOff(void) {
  ...
  //
  _GPIO_ResetBits(GPIOI, GPIO_Pin_12);
  _GPIO_ResetBits(GPIOK, GPIO_Pin_3);
  //
  // Display Off
  //
  _LTDC_Cmd(DISABLE);
...
*       _LCD_InitController
*/
static void _LCD_InitController(U32 LayerIndex) {
...
static void _LCD_DrawBitmapM565(int LayerIndex, int x, int y, const void * p, int xSize, int ySize, int BytesPerLine) {
  ...
  _DMA_DrawBitmap((void *)AddrDst, p, xSize, ySize, OffLineSrc, OffLineDst, LTDC_Pixelformat_RGB565, PixelFormatDst);
}
...
static void _LCD_DrawMemdevM565(void * pDst, const void * pSrc, int xSize, int ySize, int BytesPerLineDst, int BytesPerLineSrc) {
  ...
  OffLineDst = (BytesPerLineDst / 4) - xSize;
  _DMA_DrawBitmap(pDst, pSrc, xSize, ySize, OffLineSrc, OffLineDst, LTDC_Pixelformat_RGB565, LTDC_Pixelformat_ARGB8888);
}
...
static void _SwitchBuffersOnVSYNC(int Index, int LayerIndex) {
  ...
  for (_aPendingBuffer[LayerIndex] = 1; _aPendingBuffer[LayerIndex]; );  // Wait until _PendingBuffer is cleared by ISR
  //
  // Calculate address of buffer to be used  as visible frame buffer
  //
  ...
  // Store address into SFR
  //
  _apLayer[LayerIndex]->CFBAR &= ~(LTDC_LxCFBAR_CFBADD);
  _apLayer[LayerIndex]->CFBAR = Addr;
  //
  // Reload configuration
  //
  _LTDC_ReloadConfig(LTDC_SRCR_IMR);
...
int LCD_X_DisplayDriver(unsigned LayerIndex, unsigned Cmd, void * pData) {
  ...
  case LCD_X_INITCONTROLLER: {
    ...
    if (XPOS_0 || YPOS_0) {
      _LTDC_SetLayerPos(LayerIndex, XPOS_0, YPOS_0);
    }
#if (GUI_NUM_LAYERS > 1)
    if (XPOS_1 || YPOS_1) {
      _LTDC_SetLayerPos(LayerIndex, XPOS_0, YPOS_0);
