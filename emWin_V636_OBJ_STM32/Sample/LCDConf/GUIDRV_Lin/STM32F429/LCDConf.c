...
*       _LTDC_SetLayerPosEx
*/
static void _LTDC_SetLayerPosEx(int LayerIndex, LCD_X_SETPOS_INFO * pInfo) {
  U32 HorizontalStart, HorizontalStop, VerticalStart, VerticalStop, Addr;
  int xSize, BytesPerLine, LineLength;

  HorizontalStart = pInfo->xPos + HBP + 1;
  HorizontalStop  = pInfo->xPos + HBP + pInfo->xLen;
  VerticalStart   = pInfo->yPos + VBP + 1;
  VerticalStop    = pInfo->yPos + VBP + pInfo->yLen;
  Addr            = _aAddr[LayerIndex] + pInfo->Off;
  xSize           = LCD_GetXSizeEx(LayerIndex);
  BytesPerLine    = pInfo->BytesPerPixel * xSize;
  LineLength      = pInfo->BytesPerPixel * pInfo->xLen + 3;
  //
  ...
  // Line length
  //
  _apLayer[LayerIndex]->CFBLR = (BytesPerLine << 16) | LineLength;
  //
  // Horizontal start and stop position
  //
  _apLayer[LayerIndex]->WHPCR = (HorizontalStart | (HorizontalStop << 16));
...
static void _LCD_CopyRect(int LayerIndex, int x0, int y0, int x1, int y1, int xSize, int ySize) {
  ...
  AddrDst = _aAddr[LayerIndex] + BufferSize * _aBufferIndex[LayerIndex] + ((y0 + ySize) * _axSize[LayerIndex] + (x0 + xSize)) * _aBytesPerPixels[LayerIndex];
  OffLine = _axSize[LayerIndex] - x1;
  _DMA_Copy(LayerIndex, (void *)AddrSrc, (void *)AddrDst, x1, y1, OffLine, OffLine);
}
...
int LCD_X_DisplayDriver(unsigned LayerIndex, unsigned Cmd, void * pData) {
  ...
  if (LayerIndex < GUI_NUM_LAYERS) {
    ...
    case LCD_X_SETORG: {
      ...
      _apLayer[LayerIndex]->CFBAR = _aAddr[LayerIndex] + (p->yPos * _axSize[LayerIndex] + p->xPos) * _aBytesPerPixels[LayerIndex];
      _LTDC_ReloadConfig(LTDC_SRCR_VBR); // Reload on next blanking period
      break;
    }
